...
interface SettingsPageProps {
    ...
    updateSetting: <K extends keyof FeedbackConfigState>(key: K, value: FeedbackConfigState[K]) => void;
    saveSettings: () => void;
...
export function SettingsPage({ config, updateSetting, saveSettings, hasUnsavedChanges, isPro, loading, isAdmin = false }: SettingsPageProps) {
        ...
        <LayoutWrapper>
            <div className="max-w-4xl mx-auto space-y-8 pb-10">
                <div className="flex justify-between items-center">
                    <div>
                        <h2 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Configuration</h2>
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Manage widget behavior and appearance.</p>
                    </div>
                    ...
                    {hasUnsavedChanges && (
                        <div className="flex items-center gap-4 bg-amber-500/10 border border-amber-500/20 dark:bg-amber-500/10 dark:border-amber-500/20 px-4 py-2 rounded-lg animate-in slide-in-from-right-5 fade-in duration-300">
                            <span className="text-sm text-amber-700 dark:text-amber-200 font-medium">Unsaved changes</span>
                            <button
                ...
                <div className="grid gap-6 md:grid-cols-3">
                    ...
                    <div className="md:col-span-2 space-y-6">
                            ...
                            <div className="flex justify-between items-start z-10 relative">
                                <div>
                                    ...
                                    <div className="mt-4 text-xs font-mono text-gray-500">
                                        Project ID: <span className="text-gray-300">demo-app</span>
                                    </div>
                                </div>
                        ...
                        */}
                        ...
                        {/* Adapter Settings */}
                        <GlassCard title="Storage Adapter" description="Choose where feedback data is stored.">
                            <div className="space-y-4">
                                <div className="grid grid-cols-3 gap-2 p-1 bg-gray-100 dark:bg-black/20 rounded-lg">
                                    {(['local', 'supabase', 'console'] as const).map((id) => {
                                        const isLocked = id === 'supabase' && !isPro;
                                        return (
                                            <button
                                                key={id}
                                                onClick={() => !isLocked && updateSetting('adapterId', id)}
                                                disabled={isLocked}
                                                className={`
                                ...
                                {config.adapterId === 'supabase' && (
                                    <div className="space-y-4 pt-4 border-t border-gray-200 dark:border-white/10 animate-in slide-in-from-top-2 fade-in duration-300">
                                        <InputGroup
                                            ...
                                            onChange={(e: any) => updateSetting('supabaseUrl', e.target.value)}
                                        />
                                        <InputGroup
                                            ...
                                            onChange={(e: any) => updateSetting('supabaseKey', e.target.value)}
                                        />
                                        <div className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 p-3 rounded-lg border border-amber-200 dark:border-amber-500/20">
                                            ...
                                            Ensure your Supabase project has the <code>feedback</code> table created.
                                            Check the Admin Portal for SQL snippets.
                                        </div>
                                    </div>
                                )}
                                ...
                                {config.adapterId === 'local' && (
                                    <p className="text-xs text-gray-500 dark:text-gray-400">
                                        Feedback is saved to your browser's Local Storage. Useful for testing without a backend.
                                    </p>
                                )}

                                {config.adapterId === 'console' && (
                                    <p className="text-xs text-gray-500 dark:text-gray-400">
                                        Feedback is logged to the browser console. No data is persisted.
                                    </p>
                                )}
                            </div>
                        </GlassCard>
                    ...
                    <div className="space-y-6">
                        ...
                        <GlassCard title="Appearance">
                            <div className="space-y-6">
                                <div>
                                    <h5 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Theme Mode</h5>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Theme Color</label>
                        ...
                        <GlassCard title="Diagnostics Limits">
                            <div className="grid gap-4">
                                <InputGroup
                                    ...
                                    onChange={(e: any) => updateSetting('maxConsoleErrors', parseInt(e.target.value))}
                                    min={0}
                                    max={50}
                                    disabled={!isPro}
                                />
                                <InputGroup
                                    ...
                                    onChange={(e: any) => updateSetting('maxNetworkErrors', parseInt(e.target.value))}
                                    min={0}
                                    max={20}
                                    disabled={!isPro}
                                />
                                <InputGroup
                                    ...
                                    onChange={(e: any) => updateSetting('maxBreadcrumbs', parseInt(e.target.value))}
                                    min={0}
                                    max={50}
                                    disabled={!isPro}
                                />
                                <InputGroup
                                    ...
                                    onChange={(e: any) => updateSetting('toastDuration', parseInt(e.target.value))}
                                    min={0}
                                    step={100}
                                    disabled={!isPro}
                                />
            ...
            {isAdmin && <div className="mt-12 pt-6 border-t border-gray-200 dark:border-white/10 opacity-75 hover:opacity-100 transition-opacity">
...
function Toggle({ label, description, checked, onChange, disabled }: { label: string, description?: string, checked: boolean, onChange: (c: boolean) => void, disabled?: boolean }) {
    return (
        <div className={`flex items-start justify-between ${disabled ? 'opacity-50' : ''}`}>
            <div>
                <span className="text-sm font-medium text-gray-900 dark:text-gray-200 block">{label}</span>
                {description && <span className="text-xs text-gray-500 dark:text-gray-400 block mt-0.5 max-w-[200px]">{description}</span>}
            </div>
            <button
...
function InputGroup({ label, type = 'text', value, onChange, ...props }: any) {
    return (
        <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-1.5">{label}</label>
            <input
                type={type}
                value={value}
                onChange={onChange}
