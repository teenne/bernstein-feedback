...
export default function UserManagementPage() {
  ...
  const fetchInvites = async () => {
    try {
      const res = await fetch(`${API_URL}/api/auth/invites`, {
        headers: getAuthHeaders(),
      });
      const json = await res.json();
  ...
  const handleCancelInvite = async (id: string) => {
    try {
      await fetch(`${API_URL}/api/auth/invites/${id}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
      });
      fetchInvites();
      ...
      <div className="max-w-5xl space-y-6 pb-10">
        ...
        <GlassCard>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
            Send a signup invitation link by email. The link expires in 7 days.
          </p>
          ...
          <form onSubmit={handleSendInvite} className="flex gap-2 mb-4">
            <input
              ...
              className="flex-1 px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500/50 transition-all"
            />
            <button
              type="submit"
              disabled={inviting || !inviteEmail.trim()}
              className="px-4 py-2 text-sm font-medium bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white rounded-xl transition-colors flex-shrink-0"
            >
          ...
          {inviteResult && (
            <p
              className={`text-xs mb-4 px-3 py-2 rounded-lg ${inviteResult.kind === "success" ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400"}`}
            >
                  ...
                  .map((inv) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="ml-4 text-[11px] font-medium text-red-400 hover:text-red-500 transition-colors flex-shrink-0"
                      >
                        Cancel
                      </button>
                    </div>
          ...
          {loading ? (
            ...
            <div className="divide-y divide-gray-100 dark:divide-white/5">
              {paginated.map((user) => {
                ...
                const isLastAdmin =
                  isMainAdmin || (user.role === "admin" && adminCount <= 1);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                        className={`h-9 w-9 rounded-full flex items-center justify-center flex-shrink-0 ${color}`}
                      >
                  ...
                  <div className="flex items-center gap-0.5">
                    {Array.from({ length: totalPages }, (_, i) => {
                      const show =
                        ...
                        Math.abs(i - page) <= 1;
                      const showDot =
                        !show && (i === 1 || i === totalPages - 2);
                      if (showDot)
                        return (
                          <span
                            key={i}
                            className="text-[11px] text-gray-300 dark:text-gray-600 px-0.5"
                          >
                            …
                          </span>
                        );
